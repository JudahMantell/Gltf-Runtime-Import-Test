...
namespace Piglet
{
	public class GltfImporter
	{
		...
		protected IEnumerable<Texture2D> LoadTexture(GLTF.Schema.Texture def, int index)
		{
			...
			texture.name = string.IsNullOrEmpty(def.Name)
				? string.Format("texture_{0}", index)
				: UnityPathUtil.GetLegalAssetName(def.Name);
			...
			GLTF.Schema.Material def, int index)
		{
			...
			material.name = string.IsNullOrEmpty(def.Name)
				? string.Format("material_{0}", index)
				: UnityPathUtil.GetLegalAssetName(def.Name);
		...
		protected IEnumerator LoadMeshes()
		{
			...
			{
				var mesh = LoadMesh(_root.Meshes[i], i);
		...
		/// </summary>
		protected List<KeyValuePair<UnityEngine.Mesh, UnityEngine.Material>>
		LoadMesh(GLTF.Schema.Mesh meshDef, int meshId)
		{
			...
			for (int i = 0; i < meshDef.Primitives.Count; ++i)
			{
				...
				var meshPrimitive = LoadMeshPrimitive(primitive, meshId, i);
		...
		protected UnityEngine.Mesh LoadMeshPrimitive(
			MeshPrimitive primitive, int meshIndex, int primitiveIndex)
		{
			...
			var meshName = string.IsNullOrEmpty(_root.Meshes[meshIndex].Name)
				? string.Format("mesh_{0}_{1}", meshIndex, primitiveIndex)
				: UnityPathUtil.GetLegalAssetName(_root.Meshes[meshIndex].Name);

			mesh.name = string.Format("{0}_{1}", meshName, primitiveIndex);
		...
		protected IEnumerator<AnimationClip> LoadAnimation(GLTF.Schema.Animation animation, int index)
		{
			...
			clip.name = string.IsNullOrEmpty(animation.Name)
				? string.Format("animation_{0}", index)
				: UnityPathUtil.GetLegalAssetName(animation.Name);
