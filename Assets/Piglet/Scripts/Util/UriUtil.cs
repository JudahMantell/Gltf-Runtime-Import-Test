...
namespace Piglet
{
	...
	public class UriUtil
	{
			...
			Uri uri, Action<ulong, ulong> onProgress=null)
		{
			...
			ulong size = 0;
			foreach (var result in GetSizeInBytesEnum(uri))
			{
				size = result;
				yield return null;
			}
		...
		/// </summary>
		static public IEnumerable<Texture2D> ReadTextureEnum(string uri)
		{
		...
		/// Coroutine to load a Texture2D from a URI.
		/// </summary>
		static public IEnumerable<Texture2D> ReadTextureEnum(Uri uri)
		{
